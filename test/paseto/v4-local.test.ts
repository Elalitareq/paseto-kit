...
const local = vectors.tests.filter((t) => t.name.startsWith('4-E-') && !t['expect-fail']);

for (const v of local) {
  test(`v4.local encrypt matches vector: ${v.name}`, () => {
    const token = _encrypt(
      new LocalKey(hb(v.key)),
      u(v.payload),
      u(v.footer ?? ''),
  ...
  test(`v4.local decrypt matches vector: ${v.name}`, () => {
    ...
    expect(new TextDecoder().decode(footer)).toBe(v.footer ?? '');
  });
}
...
test('at least the expected number of local cases ran', () => {
